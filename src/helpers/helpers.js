...
const responseCookie = (res, status, statusCode, message, data, dataCookie, optionCookie) => {
  res
    .cookie('authtest', dataCookie, { ...optionCookie })
    .status(statusCode)
    .json({
      status,
      statusCode,
...
const sendVerifEmailRegister = async (token, emailTo) => {
  try {
    await mailer.sendMail({
      from: `"Ceo Test" <${process.env.NODEMAILER_AUTH_USER}>`,
      to: emailTo,
      subject: 'Verify Email Address',
      html: `${process.env.BACKEND_URL}/users/auth/verif-email/${token}`,
    });
  } catch (error) {
    console.log(error);
  }
